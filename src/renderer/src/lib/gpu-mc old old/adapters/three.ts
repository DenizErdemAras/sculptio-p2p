...
export class ThreeAdapter {
  ...
  private _syncPreview(): void {
    ...
    if (count > 0 && buf) {
      if (!this.previewMesh || (this.previewMesh as any).__gpuBuf !== buf) {
        if (this.previewMesh) { this.previewGroup.remove(this.previewMesh); (this.previewMesh.geometry as any).dispose(); }
        const geo = new this.THREE.BufferGeometry();
        this._injectBuffer(geo, buf);
        geo.boundingSphere = new this.THREE.Sphere(new this.THREE.Vector3(0,0,0), Infinity);
  ...
  private _injectBuffer(geo: import('three').BufferGeometry, gpuBuf: GPUBuffer): void {
    const floatsPerVert = this.volume.floatsPerVert;
    const intBuf = new this.THREE.InterleavedBuffer(DUMMY, floatsPerVert);
    (intBuf as any).count = 250000;
    this.backend.set(intBuf, { buffer: gpuBuf, version: 0 });
