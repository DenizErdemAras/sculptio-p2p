...
export class Volume {
  ...
  private _previewPendingReadback = false;
  ...
  previewShader(opts: PreviewShaderOptions): void {
    ...
    if (this._oldPreviewAABB) this.chunks.markDirtyInAABB(this._oldPreviewAABB.min, this._oldPreviewAABB.max);
    this.chunks.markDirtyInAABB(min, max);
    this._oldPreviewAABB = { min, max };

    const modeMap: Record<string, number> = { 'add-overlay': 1, 'remove-overlay': 2, 'combined-overlay': 3, 'direct': 4 };
    const mode = modeMap[opts.mode ?? 'direct'] ?? 4;
    this._lastPreviewMode = mode;

    const f32 = new Float32Array(16);
    const u32 = new Uint32Array(f32.buffer);
    f32[0] = opts.offset?.[0] ?? 0; f32[1] = opts.offset?.[1] ?? 0; f32[2] = opts.offset?.[2] ?? 0;
    f32[3] = opts.scale ?? 1.0;
    f32[4] = opts.multiplier ?? 1.0;
    u32[5] = mode;
    f32[6] = opts.previewMargin ?? 0.001; // Maps to previewMargin in WGSL
    f32[8]  = min[0]*this.voxelSize + this.gridOrigin[0]; f32[9]  = min[1]*this.voxelSize + this.gridOrigin[1]; f32[10] = min[2]*this.voxelSize + this.gridOrigin[2];
    f32[12] = max[0]*this.voxelSize + this.gridOrigin[0]; f32[13] = max[1]*this.voxelSize + this.gridOrigin[1]; f32[14] = max[2]*this.voxelSize + this.gridOrigin[2];
    f32[15] = opts.isoLevel ?? this._isoLevel; // Maps to isoLevel in WGSL
    this.device.queue.writeBuffer(this.previewUniBuf, 0, f32.buffer);

    // Swap ChunkManager over to the brush's active pipeline
    this.pipelines.setActivePreviewBrush(opts.brush);

    const bboxDims: Vec3 = [max[0]-min[0]+1, max[1]-min[1]+1, max[2]-min[2]+1];
    const worstCase = bboxDims[0]*bboxDims[1]*bboxDims[2]*15;
    this.previewManager.ensureCapacity(worstCase);
    this.previewManager.writeChunkUniforms(min, bboxDims);

    const encoder = this.device.createCommandEncoder();
    encoder.clearBuffer(this.previewManager.counterBuf);
    const pass = encoder.beginComputePass();
    pass.setPipeline(this.pipelines.mcGenerate); // Perfectly uses the active brush!
    pass.setBindGroup(0, this._buildPreviewSharedBindGroup());
    pass.setBindGroup(1, this.previewManager.buildBindGroup());
    pass.dispatchWorkgroups(Math.ceil(bboxDims[0]/8), Math.ceil(bboxDims[1]/8), Math.ceil(bboxDims[2]/4));
    pass.end();
    encoder.copyBufferToBuffer(this.previewManager.counterBuf, 0, this.previewManager.counterStagingBuf, 0, 4);
    this.device.queue.submit([encoder.finish()]);

    this._previewPendingReadback = true;
  }

  clearPreview(): void {
    // Modes 2 (remove), 3 (combined), and 4 (direct) all deform the main mesh!
    const needsRemesh = this._lastPreviewMode === 2 || this._lastPreviewMode === 3 || this._lastPreviewMode === 4;
    ...
    this.device.queue.writeBuffer(this.previewUniBuf, 20, new Uint32Array([0])); // mode=0
    if (this.previewManager) this.previewManager.vertCount = 0;
    this._previewPendingReadback = false;
    this._lastPreviewMode = 0;
    
    this._onPreviewUpdated?.();

    // If the main mesh was deformed by the preview (remove or combined mode), 
    // it MUST auto-update to heal the geometry now that the SDF is cleared!
    if (needsRemesh) {
      this.update(); 
    }
  }
...
async update(options: VolumeUpdateOptions = {}): Promise<void | VolumeUpdateStats> {
    ...
    // ── Preview counter readback ─────────────────────────────────────────────
    if (this._previewPendingReadback && this.previewManager) {
        const previewStart = this._now();

        this._previewPendingReadback = false;
        this._isMeshing = true;

        try {
            await this.previewManager.counterStagingBuf.mapAsync(GPUMapMode.READ, 0, 4);
            const raw = new Uint32Array(this.previewManager.counterStagingBuf.getMappedRange(0, 4).slice(0));
            this.previewManager.counterStagingBuf.unmap();

            this.previewManager.vertCount = raw[0];
            this._onPreviewUpdated?.();

            if (measure) {
                const allocatedVertices = this.previewManager.vertBuf.allocatedVerts;
                stats.preview = {
                    generatedVertices: raw[0],
                    allocatedVertices,
                    allocatedBytes: allocatedVertices * this.floatsPerVert * 4,
                    timeMs: this._now() - previewStart,
                };
            }
        } finally {
    ...
    try {
        await this._counterStagingBuf.mapAsync(GPUMapMode.READ, 0, this.chunks.chunkCount * 4);

        const counts = new Uint32Array(
            this._counterStagingBuf.getMappedRange(0, this.chunks.chunkCount * 4).slice(0),
        );
