...
export class ChunkManager {
  ...
  getMeshStats() {
    ...
    for (const chunk of this.chunks) {
      generatedVertices += chunk.info.lastVertCount;
      allocatedVertices += chunk.info.allocatedVerts;
    }

    const floatsPerVertex = this.floatsPerVert;

    const generatedBytes =
      generatedVertices * floatsPerVertex * 4;

    const allocatedBytes =
      allocatedVertices * floatsPerVertex * 4;

    return {
      generatedVertices,
      generatedTriangles: generatedVertices / 3,
